...
from .dot11 import Dot11SMState
...
class Callbacks(object):
    def __init__(self, parent):
        ...
        self.cb_recv_pkt_supp = self.recv_pkt_supp
        ...
        try:
            ...
            if packet.type == DOT11_TYPE_MANAGEMENT:
                if packet.subtype == DOT11_SUBTYPE_PROBE_REQ:  # Probe request
                    if Dot11Elt in packet:
                        ...
                        if ssid in self.parent.ssids or (Dot11Elt in packet and packet[Dot11Elt].len == 0):
                            if not (self.parent.hidden and ssid != self.parent.get_ssid()):
                                self.cb_dot11_probe_req(packet.addr2, self.parent.get_ssid())
                elif packet.subtype == DOT11_SUBTYPE_AUTH_REQ:  # Authentication
                    if packet.addr1 == self.parent.mac:  # We are the receivers
                        self.parent.sc = -1  # Reset sequence number
                        self.cb_dot11_auth(packet.addr2, 0x02)
                elif packet.subtype == DOT11_SUBTYPE_ASSOC_REQ or packet.subtype == DOT11_SUBTYPE_REASSOC_REQ:
                    if packet.addr1 == self.parent.mac:
                        self.cb_dot11_assoc_req(packet.addr2, packet.subtype)

                        # After association, start EAP session if enabled
                        if self.parent.ieee8021x:
            ...
            if packet.type == DOT11_TYPE_DATA:
                ...
                if EAP in packet:
                    ...
                    if packet[ARP].pdst == self.parent.ip.split('/')[0]:
        ...
        try:
            ...
            if BOOTP in packet:
                ...
                if DHCP in packet and packet[DHCP].options[0][1] == 5:  # DHCP ACK
                    ...
                    self.parent.arp.add_entry(client_ip, client_mac)
                ...
                self.dot11_encapsulate_ip(self.parent.arp.get_entry(client_ip), packet)

        except Exception as err:
            print("Unknown error at tun interface: %s" % repr(err))

    def dot11_probe_resp(self, source, ssid):
        probe_response_packet = self.parent.get_radiotap_header() \
                                / Dot11(subtype=5, addr1=source, addr2=self.parent.mac, addr3=self.parent.mac, SC=self.parent.next_sc()) \
                                / Dot11ProbeResp(timestamp=self.parent.current_timestamp(), beacon_interval=0x0064, cap=0x2104) \
                                / Dot11Elt(ID='SSID', info=ssid) \
                                / Dot11Elt(ID='Rates', info=AP_RATES) \
                                / Dot11Elt(ID='DSset', info=chr(self.parent.channel))
        ...
        sendp(probe_response_packet, iface=self.parent.interface, verbose=False)

    def dot11_beacon(self, ssid):
        # Create beacon packet
        beacon_packet = self.parent.get_radiotap_header()                                                    \
                     / Dot11(subtype=8, addr1='ff:ff:ff:ff:ff:ff', addr2=self.parent.mac, addr3=self.parent.mac) \
        ...
        beacon_packet.SC = self.parent.next_sc()

        # Update timestamp
        beacon_packet[Dot11Beacon].timestamp = self.parent.current_timestamp()

        # Send
        sendp(beacon_packet, iface=self.parent.interface, verbose=False)

    def dot11_auth(self, receiver, seq):
        auth_packet = self.parent.get_radiotap_header() \
                      / Dot11(subtype=0x0B, addr1=receiver, addr2=self.parent.mac, addr3=self.parent.mac, SC=self.parent.next_sc()) \
                      / Dot11Auth(seqnum=seq)

        printd("Sending Authentication (0x0B)...", Level.DEBUG)
        sendp(auth_packet, iface=self.parent.interface, verbose=False)

    def dot11_ack(self, receiver):
        ack_packet = self.parent.get_radiotap_header() \
                     / Dot11(type='Control', subtype=0x1D, addr1=receiver)

        print("Sending ACK (0x1D) to %s ..." % receiver)
        sendp(ack_packet, iface=self.parent.interface, verbose=False)

    def dot11_assoc_resp(self, receiver, reassoc):
        response_subtype = 0x01
        if reassoc == 0x02:
            response_subtype = 0x03
        assoc_packet = self.parent.get_radiotap_header() \
                       / Dot11(subtype=response_subtype, addr1=receiver, addr2=self.parent.mac, addr3=self.parent.mac, SC=self.parent.next_sc()) \
                       / Dot11AssoResp(cap=0x2104, status=0, AID=self.parent.next_aid()) \
                       / Dot11Elt(ID='Rates', info=AP_RATES)

        printd("Sending Association Response (0x01)...", Level.DEBUG)
        sendp(assoc_packet, iface=self.parent.interface, verbose=False)

    def dot11_cts(self, receiver):
        cts_packet = self.parent.get_radiotap_header() \
                     / Dot11(ID=0x99, type='Control', subtype=12, addr1=receiver, addr2=self.parent.mac, SC=self.parent.next_sc())

        printd("Sending CTS (0x0C)...", Level.DEBUG)
        sendp(cts_packet, iface=self.parent.interface, verbose=False)

    def arp_resp(self, receiver_mac, receiver_ip):
        arp_packet = self.parent.get_radiotap_header() \
                     / Dot11(type="Data", subtype=0, addr1=receiver_mac, addr2=self.parent.mac, addr3=self.parent.mac, SC=self.parent.next_sc(), FCfield='from-DS') \
                     / LLC(dsap=0xaa, ssap=0xaa, ctrl=0x03) \
                     / SNAP(OUI=0x000000, code=ETH_P_ARP) \
                     / ARP(psrc=self.parent.ip.split('/')[0], pdst=receiver_ip, op="is-at", hwsrc=self.parent.mac, hwdst=receiver_mac)

        printd("Sending ARP Response...", Level.DEBUG)
        sendp(arp_packet, iface=self.parent.interface, verbose=False)

    def dot1x_eap_resp(self, receiver, eap_code, eap_type, eap_data):
        eap_packet = self.parent.get_radiotap_header() \
                     / Dot11(type="Data", subtype=0, addr1=receiver, addr2=self.parent.mac, addr3=self.parent.mac, SC=self.parent.next_sc(), FCfield='from-DS') \
                     / LLC(dsap=0xaa, ssap=0xaa, ctrl=0x03) \
                     / SNAP(OUI=0x000000, code=0x888e) \
                     / EAPOL(version=1, type=0) \
                     / EAP(code=eap_code, id=self.parent.eap.next_id(), type=eap_type)
        ...
        sendp(eap_packet, iface=self.parent.interface, verbose=False)

    def unspecified_raw(self, raw_data):
        raw_packet = Raw(raw_data)

        printd("Sending RAW packet...", Level.DEBUG)
        sendp(raw_packet, iface=self.parent.interface, verbose=False)

    def dhcp_offer(self, client_mac, client_ip, xid):
        dhcp_offer_packet = self.parent.get_radiotap_header() \
                            / Dot11(type="Data", subtype=0, addr1="ff:ff:ff:ff:ff:ff", addr2=self.parent.mac, SC=self.parent.next_sc(), FCfield='from-DS') \
                            / LLC(dsap=0xaa, ssap=0xaa, ctrl=0x03) \
                            / SNAP(OUI=0x000000, code=ETH_P_IP) \
                            / IP(src=self.parent.ip, dst=client_ip) \
                            / UDP(sport=67, dport=68) \
                            / BOOTP(op=2, yiaddr=client_ip, siaddr=self.parent.ip, giaddr=self.parent.ip, chaddr=mac_to_bytes(client_mac), xid=xid) \
                            / DHCP(options=[('message-type', 'offer')]) \
                            / DHCP(options=[('subnet_mask', '255.255.255.0')]) \
                            / DHCP(options=[('server_id', self.parent.ip), 'end'])

        sendp(dhcp_offer_packet, iface=self.parent.interface, verbose=False)

    def dhcp_ack(self, client_mac, client_ip, xid):
        dhcp_ack_packet = self.parent.get_radiotap_header() \
                          / Dot11(type="Data", subtype=0, addr1="ff:ff:ff:ff:ff:ff", addr2=self.parent.mac, SC=self.parent.next_sc(), FCfield='from-DS') \
                          / LLC(dsap=0xaa, ssap=0xaa, ctrl=0x03) \
                          / SNAP(OUI=0x000000, code=ETH_P_IP) \
                          / IP(src=self.parent.ip, dst=client_ip) \
                          ...
                          / DHCP(options=[('server_id', self.parent.ip)]) \
                          / DHCP(options=[('lease_time', 43200)]) \
                          / DHCP(options=[('subnet_mask', '255.255.255.0')]) \
                          / DHCP(options=[('router', self.parent.ip)]) \
                          / DHCP(options=[('name_server', DEFAULT_DNS_SERVER)]) \
                          / DHCP(options=[('domain', "localdomain")]) \
                          / DHCP(options=['end'])
        sendp(dhcp_ack_packet, iface=self.parent.interface, verbose=False)
    ...
    def dot11_encapsulate_ip(self, client_mac, ip_packet):
        ...
        response_packet = self.parent.get_radiotap_header() \
                          / Dot11(type="Data", subtype=0, addr1=client_mac, addr2=self.parent.mac, SC=self.parent.next_sc(), FCfield='from-DS') \
                          / LLC(dsap=0xaa, ssap=0xaa, ctrl=0x03) \
                          / SNAP(OUI=0x000000, code=ETH_P_IP) \
                          / Raw(ip_packet)

        sendp(response_packet, iface=self.parent.interface, verbose=False)

    def dot11_to_tint(self, pkt):
        self.parent.tint.write(pkt)  # Pass to third party application for handling

    def recv_pkt_supp(self, packet):
        try:
            state = self.parent.sm.state

            if state == Dot11SMState.UNAUTHENTICATED:
                if packet.subtype == DOT11_SUBTYPE_AUTH_REQ:
                    self.parent.scanner.stop = True
                    printd("Authenticated to AP", Level.INFO)
                    state = Dot11SMState.UNASSOCIATED
            elif state == Dot11SMState.UNASSOCIATED:
                printd("Continue here", Level.INFO)

            self.parent.sm.state = state
